...
public partial class UpsertOrderForm : Form
{
    ...
    private void okButton_Click(object sender, EventArgs e)
    {
        ...
        if (_isEditing)
        {
            ordersTableAdapter.UpdateOrderQuery(
                (int)accountComboBox.SelectedValue,
                (int)carComboBox.SelectedValue,
                orderCreationDateTimePicker.Value.ToString(),
                Convert.ToDecimal(overallPriceTextBox.Text),
                (int)statusComboBox.SelectedValue,
                (int)statusComboBox.SelectedValue == _order.Status ? null : DateTime.Now.ToString(),
                _order.Id);
        }
        else
        {
            ordersTableAdapter.InsertOrderQuery(
                (int)accountComboBox.SelectedValue,
                (int)carComboBox.SelectedValue,
                orderCreationDateTimePicker.Value.ToString(),
                Convert.ToDecimal(overallPriceTextBox.Text),
